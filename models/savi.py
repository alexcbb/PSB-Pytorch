...
from .layers import SlotAttention, CoordinatePositionEmbed
import lightning as L

# TODO : finish setup SAVi here
class SAVi(nn.Module):
    ...
    def encode(self, img):
        ...
        for t in range(T):
            ...
            all_masks.append(out_dict["masks"])
        slots = torch.stack(all_slots, dim=1)  # [B, T, num_slots, slot_size]
        masks = torch.stack(all_masks, dim=1)  # [B, T, num_slots, 1, H, W]
        return slots, masks
...
# TODO : implement a unique module
class SAViModule(L.LightningModule):
    def __init__(
            self, 
            cfg
        ):
