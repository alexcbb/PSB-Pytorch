...
from models.savi import SAViModule
...
def main(cfg):
    ...
    # Logger & Callbacks
    wandb_logger = WandbLogger(project="SlotFormer", offline=cfg.offline, name=f"SAVi_{cfg.dataset_name}")
    lr_monitor = LearningRateMonitor(logging_interval='step')
    checkpoint_callback = ModelCheckpoint(
        save_top_k=3,
        monitor="train_loss",
        mode="min",
        dirpath=f"./checkpoints/savi",
        filename=f"SAVi_{cfg.dataset_name}_"+"{epoch:02d}-{train_loss:.2f}",
        save_weights_only=True, # only save the weights of the model
        every_n_train_steps=cfg.checkpoint_every_n_steps,
    )
    # Model & Data
    model = SAViModule(cfg)
